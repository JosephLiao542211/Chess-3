...
public class CardBehaviour : MonoBehaviour
{
    ...
    [SerializeField] private GameObject inspectPanel; // UI panel for inspect mode (assign in Inspector)
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("Inspect Panel not assigned in Inspector for " + cardName + ". Please assign a UI panel.");
            return; // Exit Start to avoid further null reference issues
        }
        ...
        inspectPanel.SetActive(false); // Hide inspect panel by default
    ...
    void OnMouseDown()
    {
        ...
        if (Time.time - lastClickTime < doubleClickThreshold)
        {
            ...
            return; // Exit to avoid dragging on double-click
        }
        lastClickTime = Time.time; // Record current click time
    ...
    public void SetCardData(string name, string description, int cost)
    {
        ...
        cardCost = cost;
        if (isInspecting) UpdateInspectPanel(); // Update if already inspecting
    }
